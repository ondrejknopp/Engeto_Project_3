...
def main():
    ...
    odkaz_okresu = input("Vložte odkaz okresu, ze ktereho chcete extrahovat výsledky: ").strip() # testovaci = "https://volby.cz/pls/ps2017nss/ps32?xjazyk=CZ&xkraj=8&xnumnuts=5201"
    soup_okresni = get_soup(odkaz_okresu)
    #print(soup_okresni.find_all("a"))  #tady jsou ukryté všechny potřebné adresy!
...
def get_soup(adresa):
    ...
    try:
        ...
        print("Chyba s vložením odkazu, zkontrolujte vložený odkaz!")
        sys.exit()
    return bs(r.text,"html.parser")

def vytvor_zahlavi(soup):
    part1 = ['Kód obce','Název obce','Registrovaných voličů','Vydáno obálek','Platných hlasů']
...
def get_odkazy_obci(soup_default):
    ...
    for city_path in soup_default.find_all(["a"]):
        if "obec" in str(city_path):
            ...
            adresy.append("http://volby.cz/pls/ps2017nss/"+str(city_path["href"])) #pro kazdou adresu mesta toto nize, strany do slovniku, hodnoty jsou hlasy. For kazdy mesto in seznam..
            #break
        else:
            pass
...
def vytvor_csv_soubor(balik_dat):
    nazev_souboru = "prehled_vysledku_okresu"
    ...
    soubor.close()
    print("Tvorba souboru dokončena.")
